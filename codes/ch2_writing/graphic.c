...
void putfonts16_asck(char *vram, int xsize, int x, int y, char c, unsigned short int *character_korean)
{
	int idx = 0;
	for (; *character_korean != 0x00; character_korean++) {
		...
		if(first!=0&&sec!=0&&third==0){  //종성이 없을 때
			cho_bul = cho_jong_table[1][idx_jung]; //중성에 따른 초성 벌 수 결정
			first = first*2;
			switch(cho_bul){ //각 벌 수에 따른 오프셋 설정
				case 1:first += 0x28;break;//초성 2벌 받침 없는 ㅗㅛㅡ와 결합
				case 2:first += 0x50;break;//초성 3벌 받침 없는 ㅜㅠ 와 결합
				case 3:first += 0x78;break;//초성 4벌 받침 없는 ㅘㅙㅚㅢ 와 결합
				case 4:first += 0xa0;break;//초성 5벌 받침 없는 ㅝㅞㅟ와 결합
				default:
				break;
			}
			...
			sec = sec*2;
			switch(jung_bul){
				case 0:sec += 0x140;break; //중성 1벌 ㄱㅋ와 결합
				case 1:sec += 0x16c;break; //중성 2벌 ㄱㅋ이외의 결합
			}
			jong_bul = 0;

		}else if(first!=0&&sec==0&&third==0){ // 중성과 종성이 없을때
			first = first *2;
			putfont8(vram, xsize, x, y, c, hangul + first * 16);
			putfont8(vram, xsize, x+8, y, c, hangul + (first +1) * 16);
			return;
		}else if(first!=0&&sec!=0&&third!=0){
		...
		putfont8(vram, xsize, x, y, c, hangul + sec * 16);
		putfont8(vram, xsize, x+8, y, c, hangul + (sec+1) * 16);
		char *address_thrid_1 = hangul + third * 16;
		char *address_thrid_2 = hangul + (third+1) * 16;
